...
export function UploadResultsDialog({ trigger, onSuccess }: Props) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("upload-broker-results", {
        body: { fileBase64, fileName: file.name },
      });
      if (error) throw error;
      if ((data as any)?.error) throw new Error((data as any).error);

      toast.success(`Planilha atualizada: ${(data as any).rows} linhas, ${(data as any).vendedores} vendedores`);
      setOpen(false);
      setFile(null);
      setPreview([]);
      onSuccess();
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <FileSpreadsheet className="w-5 h-5" /> Atualizar planilha de resultados
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:bg-muted/30 transition-colors">
            <input
              ...
              className="hidden"
            />
            <label htmlFor="planilha-input" className="cursor-pointer flex flex-col items-center gap-2">
