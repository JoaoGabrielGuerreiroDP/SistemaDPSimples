...
export function IndividualGoalCard({ userId, userName, selectedYear, selectedMonth, totalVendido }: IndividualGoalCardProps) {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      .from("sales_goals_individual")
      .upsert(
        { user_id: userId, mes_ref: mesRef, meta: newMeta },
  ...
  return (
    <div className="glass-card p-3 sm:p-5 space-y-3 border-l-4 border-l-[hsl(217,85%,55%)]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Target className="w-4 h-4 sm:w-5 sm:h-5 text-[hsl(217,85%,55%)]" />
          <span className="text-[10px] sm:text-sm text-muted-foreground font-medium truncate">
            Meta Individual — {userName}
          </span>
        </div>
        {isGestor && !editing && (
